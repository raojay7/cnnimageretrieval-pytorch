...
datasets_names = ['oxford5k', 'paris6k', 'roxford5k', 'rparis6k','holiday']
    ...
    for dataset in datasets: 
            ...
            if L > 1:
                ...
                for i in range(1, L + 1):
                    for j in range(1, L + 1):
                        names.append(img_name + "_" + str(i - 1) + str(j - 1) + ".jpg")
                        images_path.append(prepath+img_name + "_" + str(i - 1) + str(j - 1) + ".jpg")
            else:
        ...
        for i, img_path in enumerate(qimages):
            ...
            if L>1:
                ...
                for i in range(1, L + 1):
                    for j in range(1, L + 1):
                        qnames.append(img_name+"_"+str(i-1)+str(j-1)+".jpg")
                        qimages_path.append(prepath+img_name + "_" + str(i - 1) + str(j - 1) + ".jpg")
            else :
        ...
        vecs = extract_vectors(net, images_path, args.image_size, transform, ms=ms, msp=msp)
        print('>> {}: query images...'.format(dataset))
        qvecs = extract_vectors(net, qimages_path, args.image_size, transform, ms=ms, msp=msp)
        # convert to numpy
        vecs = vecs.numpy()
        vecs=vecs.T
